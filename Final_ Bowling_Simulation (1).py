...
scores =  [] # Gesamtpunktzahl pro Spieler
...
def update_active_player_label(): 
    if playerNames:
        sc = scores[currentPlayer] if 0 <= currentPlayer < len(scores) else 0
        activePlayerLabel.config(text=f"{playerNames[currentPlayer]} | {currentThrow}. Wurf ({throws_done[currentPlayer]}/{total_throws_per_player}) | Score: {sc}", fg=foregroundColor, bg=backgroundColor)
    else:
...
def check_collision():
    ...
    if hit_ids:
        # Pins im Canvas löschen
        for pin_id in hit_ids:
            canvas.delete(pin_id)
        # Pins aus der Liste entfernen
        pins = [p for p in pins if p[0] not in hit_ids]

        # Score des aktuellen Spielers erhöhen (Anzahl getroffener Pins)
        if 0 <= currentPlayer < len(scores):
            scores[currentPlayer] += len(hit_ids)
...
def handle_end_of_throw():
    ...
    if not any(players_active):
        activePlayerLabel.config(text="Simulation abgeschlossen!", fg=foregroundColor)
        return
...
def start_simulation():
    ...
    # Falls der Startbutton noch nicht zuvor gedrückt wurde
